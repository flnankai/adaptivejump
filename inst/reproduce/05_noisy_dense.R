#!/usr/bin/env Rscript
# Reproduces four-panel noisy dense-local power curves at 5 and 10 seconds.

library(adaptivejump)

args <- commandArgs(trailingOnly = TRUE)
output_dir <- if (length(args) >= 1L) args[[1L]] else "noisy_dense_results"
...
for (seconds in frequencies) {
  for (s in seq_len(nrow(scenarios))) {
    for (theta in theta_values) {
      ...
      message(sprintf("%ds, %s, theta=%d", seconds, scenario$label, theta))
      result <- run_jump_simulation(
        model = "dense", test = "noise", n_rep = n_rep, workers = workers,
        seed = 20261200L + index,
        simulation_args = list(
          days = 5, sampling_seconds = seconds, theta = theta, mark_sd = sqrt(400),
...
utils::write.csv(summary, file.path(output_dir, "noisy_dense_power_summary.csv"), row.names = FALSE)

for (seconds in frequencies) {
  pdf(file.path(output_dir, sprintf("noisy_dense_power_%ds.pdf", seconds)), width = 12, height = 9)
